...
; ------------------------------------------------------------------
; os_get_current_drive -- get the disk
; IN/OUT: AL = disk letter

os_set_current_drive:
	mov byte [currdrive], al
	ret
...
os_get_current_drive:
	mov al, byte [currdrive]
	ret
; ------------------------------------------------------------------
; os_get_disk_status -- gets the status of a disk from the bios
; IN/OUT: AL = disk letter / status of last operation
...
.b1:
	mov [currdrive], byte 00h			; Floppy A
	jmp .end
.b2:
	mov [currdrive], byte 01h			; Floppy B
	jmp .end
.b3:
	mov [currdrive], byte 80h			; Hard Drive C
	jmp .end
.b4:
	mov [currdrive], byte 81h			; Hard Drive D
	jmp .end
.b5:
	mov [currdrive], byte 0eh			; CD/DVD Rom E
	jmp .end
...
.errormsg1		db 	'ERROR: DRIVE NOT FOUND:', 0x00
.buffer		db 	0

; ------------------------------------------------------------------
; os_get_disk_status -- gets the status of a disk from the bios
; IN/OUT: dl = disk / status of last operation

os_get_disk_status:
	pusha
	
	clc
	mov ah, 01		; Get the status of drive
	int 13h
...
; ******************************************************************
	mov dl, [currdrive]		; Set correct device
; ******************************************************************
